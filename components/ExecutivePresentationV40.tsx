...
type DrivePhotoRow = {
  ...
  verified_at:string|null; verified_by:string|null; mapping_note:string|null
}
type PresentationPhoto = {
  id:string; project_id:string; task_id:string|null; report_date:string; phase:string; caption:string;
  image_url:string; source:'site-report'|'drive'; matched:boolean; verified:boolean; match_method?:string|null; mapping_note?:string|null;
  source_url?:string; room_no?:string|null; folder_name?:string|null
}
type CondoRoomRow = {
...
type ProjectStats = {
  ...
  completed:number; latestPhotoDate:string|null; photoCount:number; matchedPhotoCount:number; verifiedPhotoCount:number
}
type CondoMetric = {
...
function evidenceLabel(photo:PresentationPhoto,taskId?:string){
...
function evidenceRank(photo:PresentationPhoto){
  if(photo.verified) return 0
  if(photo.matched&&photo.source==='site-report') return 1
  if(photo.matched) return 2
  return 3
}
function sortEvidenceLatest(a:PresentationPhoto,b:PresentationPhoto){
  const rank=evidenceRank(a)-evidenceRank(b)
  if(rank!==0)return rank
  return b.report_date.localeCompare(a.report_date)||b.id.localeCompare(a.id)
}
...
async function loadAllDrivePhotos(s:any){
  ...
  for(let from=0;;from+=PHOTO_PAGE_SIZE){
    ...
    const batch=(result.data||[]) as DrivePhotoRow[]
...
export default function ExecutivePresentationV40(){
  ...
  useEffect(()=>{
    ...
    const load=async()=>{
      ...
      const sitePhotos:PresentationPhoto[]=reportPhotos.flatMap(photo=>{
        ...
        return [{id:`report-${photo.id}`,project_id:report.project_id,task_id:item?.schedule_task_id||null,report_date:report.report_date,phase:photo.phase||'other',caption:photo.caption||item?.work_item||'Site photo',image_url:url,source:'site-report' as const,matched:Boolean(item?.schedule_task_id),verified:false,match_method:'site-report'}]
      })
      const drivePhotos:PresentationPhoto[]=(dp as DrivePhotoRow[]).map(photo=>({
        ...
        image_url:`/api/drive-photo?fileId=${encodeURIComponent(photo.drive_file_id)}`,source:'drive' as const,matched:Boolean(photo.schedule_task_id),verified:Boolean(photo.verified_at),match_method:photo.match_method,mapping_note:photo.mapping_note,source_url:photo.drive_url,
  ...
  const pickPhotos=(projectId:string,taskId?:string)=>{
    const projectPhotos=photosInPeriod.filter(p=>p.project_id===projectId)
    const direct=taskId?projectPhotos.filter(p=>p.task_id===taskId).slice().sort(sortEvidenceLatest):[]
    const fallback=taskId?projectPhotos.filter(p=>!p.task_id).slice().sort((a,b)=>b.report_date.localeCompare(a.report_date)||b.id.localeCompare(a.id)):[]
  ...
  const currentDirectCount=currentTask?currentPhotos.filter(p=>p.task_id===currentTask.id).length:0
  const currentFallbackCount=currentTask?currentPhotos.filter(p=>!p.task_id).length:0
  const currentVerifiedCount=currentTask?currentPhotos.filter(p=>p.task_id===currentTask.id&&p.verified).length:0
    ...
    try{
      ...
      for(const project of exportProjects){
        ...
        for(let idx=0;idx<projectTasks.length;idx++){
          ...
          if(taskPhotos.length){ const verified=taskPhotos.filter(p=>p.task_id===task.id&&p.verified).length; const fallback=taskPhotos.filter(p=>!p.task_id).length; slide.addText(`${verified?`Verified ${verified} รูป`:'ยังไม่มี Verified Photo'}${fallback?` • Plot fallback ${fallback} รูป`:''}`,{x:6.15,y:6.58,w:6.25,h:.22,fontSize:8.5,color:verified?'247951':'7A6A42',align:'right'}) }
  ...
  return <AppShell>
    <PageHeader title="Executive Presentation" subtitle="Villa / Mirage ใช้ Progress Sheet • รูปหลักฐานเรียง Verified Task Photo → Task Match → Plot fallback • Above Condo ใช้ Defect / Handover Status" />
    <section className="panel ep-filter">
      ...
      <div className="ep-meta"><span>Schedule Sync: <b>{formatDateTime(latestScheduleSync)}</b></span><span>Photo Index Sync: <b>{formatDateTime(latestPhotoSync)}</b></span><span>รูปช่วงนี้: <b>{photosInPeriod.length}</b></span><span>Verified: <b>{photosInPeriod.filter(p=>p.verified).length}</b></span><span>งานตามแผน: <b>{periodTasks.length}</b></span><span>Condo Status: <b>{condoRooms.length} ห้อง</b></span></div>
    </section>
    ...
    {loading?<div className="panel">กำลังเตรียม Executive Presentation…</div>:view==='overview'?<section className="ep-overview">
      ...
      {selectedProject&&selectedBuilding&&selectedCondoMetric?<>
        ...
        <section className="ep-photo-section"><div className="ep-photo-head"><div><b>{photoMode==='latest'?'รูปล่าสุด':'Before–After'}</b><span>วันที่รูปล่าสุด: {imageDateLabel(currentPhotos[0]?.report_date)} • Verified {currentVerifiedCount} รูป</span></div>{currentDriveUrl&&<a href={currentDriveUrl} target="_blank" rel="noreferrer">เปิด Picture Progress ↗</a>}</div>{currentFallbackCount>0&&<div className="ep-fallback-note">{currentDirectCount>0?`รูปตรง Task มี ${currentDirectCount} รูป — เติมรูปภาพรวมระดับ Plot ที่ยังไม่ผูก Task อีก ${currentFallbackCount} รูป`:'ยังไม่มีรูปที่จับคู่ตรง Task นี้ — แสดงเฉพาะรูปภาพรวมระดับ Plot ที่ยังไม่ผูกกับ Task เป็น fallback'}</div>}{currentPhotos.length?<div className={`ep-photo-grid count-${currentPhotos.length}`}>{currentPhotos.map(photo=><figure key={photo.id}><img src={photo.image_url} alt={photo.caption}/><figcaption><div><b>{phaseLabel(photo.phase,photo.task_id===currentTask.id)}</b><span>{evidenceLabel(photo,currentTask.id)}</span></div><span>{imageDateLabel(photo.report_date)}</span></figcaption></figure>)}</div>:<div className="ep-photo-empty"><b>ยังไม่มีรูปประกอบในช่วงที่เลือก</b><span>Photo Index จะเติมรูปจาก Picture Progress เมื่อ Sync เข้ามา</span></div>}</section></div></article></>:<div className="panel">ไม่มีงานในช่วงวันที่ที่เลือก</div>}
    </div>}
